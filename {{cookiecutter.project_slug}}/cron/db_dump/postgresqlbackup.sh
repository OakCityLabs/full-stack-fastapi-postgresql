...
DBHOST=${POSTGRES_SERVER-localhost}

# List of DBNAMES for Daily/Weekly Backup e.g. "DB1 DB2 DB3"
DBNAMES=${POSTGRES_DB-all}
...
MAILADDR=${MAILADDR-"{{ cookiecutter.first_superuser }}"}
    ...
    for db in $1 ; do
        if [ -n "$SU_USERNAME" ]; then
            if [ "$db" = "$GLOBALS_OBJECTS" ]; then
                su $SU_USERNAME -l -c "pg_dumpall $PGHOST --no-role-passwords --globals-only" >> $2
            else
                su $SU_USERNAME -l -c "pg_dump $PGHOST $OPT $db" >> $2
            fi
        else
            if [ "$db" = "$GLOBALS_OBJECTS" ]; then
                pg_dumpall --username=$USERNAME $PGHOST --no-role-passwords --globals-only >> $2
            else
